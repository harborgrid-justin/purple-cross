...
export class FeedbackService {
  ...
  async getFeedback(id: string) {
    const feedback = await this.prisma.clientFeedback.findUnique({
      where: { id },
    });
  ...
  async updateFeedback(id: string, data: any) {
    return this.prisma.clientFeedback.update({ where: { id }, data });
  }

  async deleteFeedback(id: string) {
    return this.prisma.clientFeedback.delete({
      where: { id },
    });
  }

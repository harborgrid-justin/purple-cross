...
describe('Appointment Workflow (E2E)', () => {
    ...
    it('should complete an appointment workflow', async () => {
        ...
        const createResponse = await (0, supertest_1.default)(app).post('/api/appointments').send(appointmentData);
        expect(createResponse.status).toBe(201);
        expect(createResponse.body.status).toBe('scheduled');
        const appointmentId = createResponse.body.id;
        const getResponse = await (0, supertest_1.default)(app).get(`/api/appointments/${appointmentId}`);
        expect(getResponse.status).toBe(200);
        expect(getResponse.body.id).toBe(appointmentId);
        const completeResponse = await (0, supertest_1.default)(app).patch(`/api/appointments/${appointmentId}/complete`);
    ...
    it('should validate appointment data', async () => {
        ...
        const response = await (0, supertest_1.default)(app).post('/api/appointments').send(invalidData);

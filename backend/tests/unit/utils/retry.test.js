...
describe('retry', () => {
    ...
    it('should succeed on first attempt', async () => {
        ...
        const result = await (0, retry_1.retry)(mockFn, {
            maxAttempts: 3,
            initialDelay: 100,
            name: 'test',
        });
    ...
    it('should throw error after max attempts', async () => {
        ...
        await expect((0, retry_1.retry)(mockFn, {
            maxAttempts: 3,
            initialDelay: 100,
            name: 'test',
...
describe('retryWithJitter', () => {
    it('should succeed on first attempt', async () => {
        const mockFn = jest.fn().mockResolvedValue('success');
        const result = await (0, retry_1.retryWithJitter)(mockFn, {
            maxAttempts: 3,
            initialDelay: 100,
            name: 'test',
        });
    ...
    it('should add randomness to delay (jitter)', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const startTime = Date.now();
            await (0, retry_1.retryWithJitter)(mockFn, {
                maxAttempts: 2,
                initialDelay: 100,
                name: 'test',
            });

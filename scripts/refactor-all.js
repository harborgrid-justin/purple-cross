...
const CONSTANTS_IMPORT_SERVICES = "import { HTTP_STATUS, ERROR_MESSAGES, PAGINATION, QUERY_MODE, SORT_ORDER, FIELDS, QUERY_LIMITS, STATUS } from '../constants';";
...
const SERVICE_REPLACEMENTS = [
    ...
    { pattern: /throw new AppError\('([^']+) not found', 404\)/g, replacement: "throw new AppError(ERROR_MESSAGES.NOT_FOUND('$1'), HTTP_STATUS.NOT_FOUND)" },
    { pattern: /throw new AppError\("([^"]+) not found", 404\)/g, replacement: "throw new AppError(ERROR_MESSAGES.NOT_FOUND('$1'), HTTP_STATUS.NOT_FOUND)" },
    { pattern: /throw new AppError\('([^']+) already exists', 400\)/g, replacement: "throw new AppError(ERROR_MESSAGES.ALREADY_EXISTS('$1'), HTTP_STATUS.BAD_REQUEST)" },
    { pattern: /throw new AppError\("([^"]+) already exists", 400\)/g, replacement: "throw new AppError(ERROR_MESSAGES.ALREADY_EXISTS('$1'), HTTP_STATUS.BAD_REQUEST)" },

    // Pagination
    { pattern: /\bpage = 1, limit = 20\b/g, replacement: 'page = PAGINATION.DEFAULT_PAGE, limit = PAGINATION.DEFAULT_LIMIT' },
    ...
    // Order by patterns
    { pattern: /orderBy: \{ createdAt: 'desc' \}/g, replacement: 'orderBy: { [FIELDS.CREATED_AT]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ createdAt: "desc" \}/g, replacement: 'orderBy: { [FIELDS.CREATED_AT]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ visitDate: 'desc' \}/g, replacement: 'orderBy: { [FIELDS.VISIT_DATE]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ visitDate: "desc" \}/g, replacement: 'orderBy: { [FIELDS.VISIT_DATE]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ startTime: 'desc' \}/g, replacement: 'orderBy: { [FIELDS.START_TIME]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ startTime: "desc" \}/g, replacement: 'orderBy: { [FIELDS.START_TIME]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ invoiceDate: 'desc' \}/g, replacement: 'orderBy: { [FIELDS.INVOICE_DATE]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ invoiceDate: "desc" \}/g, replacement: 'orderBy: { [FIELDS.INVOICE_DATE]: SORT_ORDER.DESC }' },
    { pattern: /orderBy: \{ name: 'asc' \}/g, replacement: 'orderBy: { [FIELDS.NAME]: SORT_ORDER.ASC }' },
    { pattern: /orderBy: \{ name: "asc" \}/g, replacement: 'orderBy: { [FIELDS.NAME]: SORT_ORDER.ASC }' },
